...
import { MODAL_CONTENT, MODAL_BUTTON, TOAST_BOTTOM_OFFSET, QUERY_KEYS, MESSAGE } from '@/constants';
  ...
  const handleDeleteLetter = useCallback(
    async (letterIdx: number) => {
      try {
        ...
        await delay(100);
        showToast(MESSAGE.LETTER.DELETED, TOAST_BOTTOM_OFFSET.HOME_SCREEN);
      } catch (error) {
        await delay(100);
  ...
  const handleOpenDeleteModal = useCallback(
    (letterIdx: number) => {
      const modalContent: AlertModalContent = {
        ...
        buttons: [
          {
            text: MODAL_BUTTON.COMMON.CLOSE,
